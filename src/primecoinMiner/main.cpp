   ...
   else if( workData.protocolMode == MINER_PROTOCOL_XPUSHTHROUGH )
{
	// printf("Queue share\n");
		...
		//CheckPrimeProofOfWork(phash, primecoinBlock->nBits, primecoinBlock->mpzPrimeChainMultiplier.get_mpz_t(), nChainType, nChainLength);
		//exit(0);
...
void jhMiner_printHelp()
{
	...
	cout << "  -g <num>			  Index of gpu to use, default is 0" << endl;
	cout << "  -pd				  Disable Pings" << endl;
...
void jhMiner_parseCommandline(int argc, char **argv)
{
	...
	while( cIdx < argc )
	{
		...
		else if( memcmp(argument, "-pd", 4) == 0)
		{
			commandlineInput.ping=0;
		}
		...
		else if( memcmp(argument, "-g", 3)==0 )
		{
			// -g
			if( cIdx >= argc )
			{
				cout << "Missing gpu index after 0" << endl;
				exit(0);
			}
			commandlineInput.gpu =  atoi(argv[cIdx]);
			cIdx++;
		}
 	...
 	else if( memcmp(argument, "-profile", 8)==0 )
      {
            commandlineInput.profile = true;

      }

 	else if( memcmp(argument, "-ptx", 4)==0 )
      {
            commandlineInput.ptx = true;

      }

 	else if( memcmp(argument, "-xpt5", 5)==0 )
      {
            commandlineInput.xpt6 = false;

      }

	else if( memcmp(argument, "-noping", 7)==0 )
      {
            commandlineInput.usePing = false;
...
void PrintCurrentSettings()
{
	...
	cout << "Gpu Index (-g): " << commandlineInput.gpu << endl;
...
int main(int argc, char **argv)
{
	...
	commandlineInput.numThreads = 1;//std::max(getNumThreads(), 1);
	commandlineInput.gpu = 0;
	commandlineInput.ping = 1;
   ...
   commandlineInput.enableCacheTunning = false;
	...
	commandlineInput.profile = false;
	commandlineInput.ptx = false;
	commandlineInput.xpt6 = true;
	commandlineInput.usePing = true;
